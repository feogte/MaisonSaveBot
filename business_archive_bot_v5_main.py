...
async def spam_business(bc_id: str, chat_id: int, count: int):
...
async def private_text_router(message: Message):
    if message.from_user and message.from_user.id == OWNER_ID:
        ...
        # If admin is in FAQ-label editing mode, consume the text.
        # We use a lightweight DB setting as a state flag.
        if get_setting("faq_edit_mode", "0") == "1":
            ...
            await message.answer(
                f"✅ Текст кнопки FAQ изменён на:\n<b>{html.escape(label)}</b>"
            )
            return


# Replaces the callback's state flag after the editor prompt.
# We cannot set it before the callback above without a separate state handler,
# so this callback is overridden below by a small initialization hook.
...
async def send_deleted_notification(
    ...
    # No ordinary media is archived, so only text/caption can be restored.
    # Still send a notification for an unknown/deleted text message.
    sent = await bot.send_message(
        owner_id,
        "<b>Удаленное сообщение</b>\n"
...
# ---------------- ADMIN FAQ STATE PATCH ----------------

# The callback above asks for new text. Set the state immediately when the
# callback is invoked. This handler is registered after the generic callback
# and therefore won't run because the generic handler matches first; instead
# we use a dedicated callback handler with a more specific filter here.
@dp.callback_query(F.data == "admin_faq_text")
async def admin_faq_text_state(callback: CallbackQuery):
    if callback.from_user.id != OWNER_ID:
        await callback.answer("Доступ запрещён.", show_alert=True)
        return
    ...
    await callback.message.edit_text(
        "<b>✏️ Изменение текста FAQ</b>\n\n"
        "Теперь отправь следующим сообщением новый текст кнопки.\n"
