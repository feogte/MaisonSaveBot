...
async def edit_screen(call: CallbackQuery, screen: str, caption: str, markup=None):
    """Replace the current message media with the selected screen photo."""
    photo_id = PHOTO_IDS.get(screen, PHOTO_ID)
    try:
        media = InputMediaPhoto(media=photo_id, caption=caption)
        await call.message.edit_media(media=media, reply_markup=markup)
    except Exception:
        # If the current message is text-only, edit_media cannot be used.
        # Delete it and send the correct photo screen.
        try:
