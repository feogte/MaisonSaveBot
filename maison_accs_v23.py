...
async def adm_product_accounts(call: CallbackQuery):
    ...
    for index, product in enumerate(products):
        rows.append([
            InlineKeyboardButton(
                text=f"{product['value']} • {product['price']:g} ₽ • {product['quantity']} шт.",
...
async def adm_delete_account(call: CallbackQuery):
    ...
    for i, item in enumerate(products):
        rows.append([
            InlineKeyboardButton(
                text=f"{item['value']} • {item['price']:g} ₽ • {item['quantity']} шт.",
...
async def adm_balance_user_id(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Текущий баланс: {user['balance']:g} ₽\n\n"
...
async def adm_balance_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"{'Добавлено' if action == 'add' else 'Удалено'}: {amount:g} ₽\n"
        f"Новый баланс: {updated['balance']:g} ₽",
        reply_markup=admin_kb()
    )
    await state.clear()
