...
def main_kb():
    ...
    if db.get_account_stock() > 0:
        rows.append([InlineKeyboardButton(text="Купить аккаунты", callback_data="accounts")])
    if db.get_star_stock() > 0:
        rows.append([InlineKeyboardButton(text="Купить звезды", callback_data="stars")])
    rows.extend([
        [
...
async def balance(call: CallbackQuery):
    if not await is_subscribed(call.bot, call.from_user.id):
        ...
        return
    user = db.get_user(call.from_user.id)
    await edit_screen(call, "balance", text("balance", balance=user["balance"]), balance_kb())
    await call.answer()
